...
This documentation outlines how to expose custom runtime data and tools from your web application to Brave DevTools for Agents.

## Overview

Third-party developer tools enable your web application to expose internal state, component hierarchies, or specific debug data that cannot be deduced through static analysis. This allows Brave DevTools for Agents to provide richer, more actionable context to AI agents during debugging sessions.

## How It Works: Tool Discovery

Brave DevTools for Agents uses an event-based mechanism to discover tools exposed by the page. The process follows these steps:

1.  **Event Dispatch:** Brave DevTools for Agents dispatches a `devtoolstooldiscovery` event on the global `window` object.
...
_Note: Brave DevTools for Agents requests this list automatically after page navigations (e.g., `new_page`, `navigate_page`) or when explicitly requested via the `list_3p_developer_tools()` MCP tool._
...
Once discovered, MCP clients can execute your tools through Brave DevTools for Agents using:
