...
* sync with Chrome DevTools MCP 1.8.0 plus the commits that landed on upstream `main` after
  that release: a 1.2gb default performance trace buffer, `--allow-unrestricted-paths` on by
  default for the CLI, no DevTools frontend Audits subscription, and the memory tools now
  spelling out the flag they need
* `pageId` is required on page-scoped tools by default, replacing
  `--experimentalPageIdRouting`; pass `--no-page-id-routing` to keep targeting the selected page
* CLI and MCP option definitions moved from `src/bin` to `src/config`
