...
The Brave DevTools MCP project takes security very seriously. Please use [GitHub private vulnerability reporting](https://github.com/triuzzi/brave-devtools-mcp/security/advisories/new) to report security issues in this fork. Report browser vulnerabilities directly to [Brave](https://hackerone.com/brave).
...
We appreciate feedback and suggestions from developers on how this tool can make it easier for them to build a more secure user experience, but will treat these exclusively as feature requests, and not vulnerabilities in brave-mcp itself.

### MCP roots

`brave-mcp` supports [MCP roots](https://modelcontextprotocol.io/specification/2025-06-18/client/roots). If the client specifies them, the MCP server will check the roots when accessing files.
